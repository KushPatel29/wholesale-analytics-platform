...
@pytest.fixture
def seed_customers_kpis_v3(tmp_path, monkeypatch):
    rows = [
        {
            "Date": "2025-02-10",
    ...
    parquet_path = tmp_path / "fact_customers_kpis_v3.parquet"
...
def _scope_sales(rep_ids: list[str]):
    return {
        ...
        "scope_hash": "scope-sales-kpis-v3",
...
def _scope_admin():
    return {
        ...
        "sales_rep_ids": [],
        "allowed_count": 0,
        "scope_hash": "scope-admin-kpis-v3",
        "permissions_version": "1",
        "user_id": 1,
        "role": "admin",
    }
...
def test_customers_kpis_v3_rbac_scope_table_and_export(app_client, seed_customers_kpis_v3, monkeypatch):
...
def test_customers_kpis_v3_export_parity_matches_filtered_rows(app_client, seed_customers_kpis_v3, monkeypatch):
...
def test_customers_kpis_v3_segment_logic_new_and_lost(app_client, seed_customers_kpis_v3, monkeypatch):
    monkeypatch.setattr("app.services.filters_service.scope_from_user", lambda _u: _scope_admin())
    monkeypatch.setitem(app_client.application.config, "CUSTOMERS_KPIS_V3", True)
    query = {"start": "2025-03-01", "end": "2025-03-31", "page_size": 100, "export_all": "1"}
    bundle_resp = app_client.get("/api/customers/bundle", query_string=query)
    ...
    assert by_customer["C_NEW"].get("delta_revenue_status") == "new"


def test_customers_kpis_v3_nrr_grr_sanity_bounds(app_client, seed_customers_kpis_v3, monkeypatch):
    monkeypatch.setattr("app.services.filters_service.scope_from_user", lambda _u: _scope_admin())
    monkeypatch.setitem(app_client.application.config, "CUSTOMERS_KPIS_V3", True)
...
def test_customers_kpis_v3_chart_exports_available(app_client, seed_customers_kpis_v3, monkeypatch):
    monkeypatch.setattr("app.services.filters_service.scope_from_user", lambda _u: _scope_admin())
    monkeypatch.setitem(app_client.application.config, "CUSTOMERS_KPIS_V3", True)
    query = {"start": "2025-03-01", "end": "2025-03-31", "format": "csv", "page": "kpis"}

    lifecycle_resp = app_client.get("/customers/export", query_string={**query, "dataset": "lifecycle_funnel"})
    assert lifecycle_resp.status_code == 200
    lifecycle_df = _csv_frame(lifecycle_resp)
    assert len(lifecycle_df.index) >= 4

    composition_resp = app_client.get("/customers/export", query_string={**query, "dataset": "revenue_composition"})
    assert composition_resp.status_code == 200
    composition_df = _csv_frame(composition_resp)
    assert len(composition_df.index) >= 4


def test_customers_kpis_v3_page_renders_with_flag(app_client, seed_customers_kpis_v3, monkeypatch):
    monkeypatch.setattr("app.services.filters_service.scope_from_user", lambda _u: _scope_admin())
    monkeypatch.setitem(app_client.application.config, "CUSTOMERS_KPIS_V3", True)
    ...
    assert "Executive Scorecard" in body
    assert "Customer Lifecycle Funnel" in body
    assert body.count("customer-stable-chart-frame") >= 2
    assert "maintainAspectRatio: false" in body
    assert "resizeDelay: 160" in body


def test_customers_kpis_v3_flag_off_falls_back_to_v2(app_client, seed_customers_kpis_v3, monkeypatch):
    monkeypatch.setattr("app.services.filters_service.scope_from_user", lambda _u: _scope_admin())
    monkeypatch.setitem(app_client.application.config, "CUSTOMERS_KPIS_V3", False)
