...
def demo_logins_enabled() -> bool:
    ...
    Tied to DEMO_WARMUP, which is set only by the demo container image, so a
    real deployment never prints credentials to an anonymous visitor.
    """
...
def demo_login_hints() -> list[dict[str, str]]:
    ...
    return [
        {"username": username, "note": str(spec.get("note") or ""), "role": str(spec.get("role") or "")}
