...
— one click on **Explore demo** and you are in, read-only, no signup. The login
page also lists six other accounts and what each is allowed to see: sign in as
`rep.dana` and watch every figure on every page narrow to one rep's book. It
sleeps when idle, so the first request can take about a minute.
